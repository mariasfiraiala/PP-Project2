...
insertTree tree trees = case trees of
    [] -> [tree]
    EmptyTree:xTree -> tree:xTree
    node:xTree -> EmptyTree:insertTree (attach node tree) xTree
...
insert prio key heap@(BinomialHeap s t) = BinomialHeap (s + 1) (insertTree (Node prio key []) t)
...
findMin heap@(BinomialHeap s t) = case list of
    [] -> Nothing
    _ -> Just $ minimumBy (compare `on` fst) list
    where
        list = [(p, k) | Node p k c <- t]
...
mergeTrees trees1 trees2 = snd varMapAccumL ++ [fst varMapAccumL] where
        addTrees carry treePair = case treePair of
            (EmptyTree, EmptyTree) -> (EmptyTree, carry)
            (EmptyTree, node@(Node {})) -> case carry of
                EmptyTree -> (EmptyTree, node)
                _ -> (attach node carry, EmptyTree)
            (node@(Node {}), EmptyTree) -> case carry of
                EmptyTree -> (EmptyTree, node)
                _ -> (attach node carry, EmptyTree)
            (node1@(Node {}), node2@(Node {})) -> (attach node1 node2, carry)
        varMapAccumL = mapAccumL addTrees EmptyTree $ zipExtend EmptyTree EmptyTree trees1 trees2
...
merge heap1@(BinomialHeap s1 t1) heap2@(BinomialHeap s2 t2) = BinomialHeap (s1 + s2) (mergeTrees t1 t2)
...
removeMin (BinomialHeap s c) = BinomialHeap (s - 1) newChildren where
    cmpBinomialTree EmptyTree EmptyTree = EQ
    cmpBinomialTree EmptyTree _ = GT
    cmpBinomialTree _ EmptyTree = LT
    cmpBinomialTree (Node p1 _ _) (Node p2 _ _) = compare p1 p2
    isolatedTrees = isolate EmptyTree c
    (toRemove, remaining) = minimumBy (cmpBinomialTree `on` fst) isolatedTrees
    newChildren = mergeTrees remaining (reverse (children toRemove))
...
{-
    ...
    *
    ...
    1 ('a')
      ...
      2 ('b')
-}
showRec :: (Show p, Show k)  => [Char] -> BinomialTree p k -> [Char]
showRec _ EmptyTree = "*"
showRec spaces (Node p k c) = spaces ++ show p ++ " (" ++ show k ++ ")\n" ++ intercalate "" (map (showRec (spaces ++ "  ")) c)

instance (Show p, Show k) => Show (BinomialTree p k) where
    show EmptyTree = "*"
    show tree = init $ showRec "" tree
...
instance (Show p, Show k) => Show (BinomialHeap p k) where
    show (BinomialHeap s t) = intercalate "\n" (map show t)
...
instance Functor (BinomialHeap p) where
    ...
    fmap f (BinomialHeap s t) = BinomialHeap s (map (fmap f) t)
...
instance Foldable (BinomialTree p) where
    ...
    foldr f acc (Node p k c) = f k $ foldr (flip (foldr f)) acc c
